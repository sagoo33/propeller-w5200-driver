...
namespace options
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            if (cnt < 1)
            {
                Console.WriteLine("USAGE: options remotefilename [localfilename]");
                Console.WriteLine("EXAMPLE: options http://192.168.1.117/upload/test.txt");
                Console.WriteLine("EXAMPLE: options http://192.168.1.117/upload/test.txt c:\\testoptions.txt");
            }
            else
            {
                string localfile = "";
                ...
                Console.WriteLine(GetOptions(args[0], localfile));
            }
            //Console.ReadKey();
        }

        private static string GetOptions(string remotefile, string localfile)
        {
            string sReturn = "";
            try
            {
